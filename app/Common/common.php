...
function attach($attach, $type) {
    if (false === strpos($attach, 'http://')) {
        //本地附件
        return __ROOT__ . '/' . C('ht_attach_path') . $type . '/' . $attach;
        //远程附件
        //todo...
    } else {
        //URL链接
...
function gets_info($iid, $flag = false) {
    ...
    $data = is_json($content);
		...
		$htian_https = new htian_https();
		$htian_https->fetch($info['descInfo']);
		$source = $htian_https->results;
		if(!$source){
