...
class userModel extends Model
{
    ...
    public function union_reg($date) {
        ...
        if (D('union')->where(array('ip'=>$ip))->find()) {
			...
			if(!$union_user['ouid']){
				$union_date['score']	= C('ht_score_rule.union_reg');
				$union_date['ouid']		= $date['uid'];
				$union_date['ousername']= $date['username'];
				D('union')->where(array('ip'=>$ip))->save($union_date);

				//积分增加
				$this->where(array('id'=>$union_user['uid']))->setInc('score',C('ht_score_rule.union_reg'));
				$this->where(array('id'=>$union_user['uid']))->setInc('invite_friend_count');
				//添加增加积分记录
				$score_log_date['uid']		= $union_user['uid'];
				$score_log_date['uname']	= $union_user['username'];
				$score_log_date['action']	= 'union_reg';
				$score_log_date['score']	= C('ht_score_rule.union_reg');
				D('score_log')->create($score_log_date);
				D('score_log')->add();

				$union_date['ouid']		= $date['uid'];
				$union_date['ousername']= $date['username'];
				$this->where(array('id'=>$union_date['ouid']))->save(array('parent_uid'=>$union_user['uid']));
				D('union')->where(array('ip'=>$ip))->save($union_date);
			}
