...
class itemsModel extends Model
{
    ...
    /**
    * 判断该单品是否已经抓取成功了
    * @author 后天网络  2014年7月24日, AM 10:23:26
    ...
    */
    ...
    /**
    * 查询出来所有的分类
    * @author 后天网络  2014年7月24日, AM 11:59:53
    */
     ...
     * ajax发布商品
     */
    public function ajax_ht_publish($item) {
        //是否存在
		//p($item);
        if ($this->where(array('num_iid'=>$item['num_iid']))->count()) {
			if($item['recid']==1){
	...
	public function get_tags_by_title($title, $num=10){
        ...
        $pscws = new PSCWS4();
        $pscws->set_dict(HT_DATA_PATH . 'scws/dict.utf8.xdb');
        $pscws->set_rule(HT_DATA_PATH . 'scws/rules.utf8.ini');
