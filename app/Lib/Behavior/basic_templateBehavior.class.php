    ...
    private function parseTemplateFile($templateFile) {
        ...
        if(''==$templateFile) {
            ...
            if(!file_exists_case($templateFile) && C('DEFAULT_THEME') && C('BASIC_THEME')) {
                ...
                $ht_default_theme = C('BASIC_THEME');
                $group = defined('GROUP_NAME') ? GROUP_NAME . '/' : '';
                $ht_theme_path = TMPL_PATH . $group . $ht_default_theme . '/';
                $templateFile = $ht_theme_path.MODULE_NAME.(defined('GROUP_NAME')?C('TMPL_FILE_DEPR'):'/').ACTION_NAME.C('TMPL_TEMPLATE_SUFFIX');
            }
