...
class oauth {
    ...
    public function __construct($type) {
        ...
        //导入接口文件
        include_once LIB_PATH . 'Htlib/oauth/' . $type . '/' . $type . '.php';
    ...
    public function callbackLogin($request_args) {
        ...
        if ($bind_user) {
            ...
            if (M('user')->where(array('username' => $user['keyname']))->count()) {
                $user['ht_user_name'] = $user['keyname'] . '_' . mt_rand(99, 9999);
            } else {
                $user['ht_user_name'] = $user['keyname'];
            }
			$username = $user['ht_user_name'];
            $user['ht_user_name'] = urlencode($user['ht_user_name']);
            ...
            $user['type'] = $this->_type;
			 ...
			 //连接用户中心
			$passport = new passport(C('ht_integrate_code'));
           // $passport = $this->_user_server();
            ...
            if ($user['keyavatar_big']) {
                ...
                $avatar_temp_root = C('ht_attach_path') . 'avatar/'.$avatar_dir;
                $file_name = md5($uid) . '_100.jpg';

                mkdir($avatar_temp_root);
                $image_content = Http::fsockopenDownload($user['keyavatar_big']);
                file_put_contents($avatar_temp_root . $file_name, $image_content);
                $user['temp_avatar'] = $avatar_temp_root . $file_name;
            }			
    ...
    public function callbackBind($request_args) {
        ...
        $ht_user = session('user_info');
        $user = $this->_om->getUserInfo($request_args);
        $bind_user = $this->_checkBind($this->_type, $user['keyid']);
        if ($bind_user['uid'] && $bind_user['uid'] != $ht_user['id']) {
        ...
        $user['ht_uid'] = $ht_user['id'];
		$user['ht_username'] = $ht_user['username'];
        $this->bindUser($user);
        return U('user/bind');
    }
    ...
    private function _oauth_visitor() {
        include_once (HTIAN_PATH . 'app/Lib/Htlib/user_visitor.class.php');
