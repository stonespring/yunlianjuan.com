...
class wechat_oauth
{
    ...
    public function __construct($setting) {
        ...
        //组装回调地址
        $this->redirect_uri =C('ht_site_url').'/index.php/m?m=oauth&a=callback&mod=wechat&tt=ss';//'/index.php?m=oauth&a=callback&mod=qq&tt=ss';//U('oauth/callback', array('mod'=>'qq'), '', '', true);
    ...
    function getAuthorizeURL() {
        ...
        $oauth = new WechatTOAuthV2($this->setting['app_key'], $this->setting['app_secret']);
    ...
    public function getUserInfo($request_args) {
     ...
     //   $openid = $oauth->getOpenid($token["access_token"]);
        ...
        $result['keyavatar_big'] = urldecode($user['headimgurl']);
    ...
    public function send($bind_user, $data) {
        ...
        $client = new WechatTOAuthV2($this->setting['app_key'], $this->setting['app_secret']);
        try {
