...
class itemTag {    
    ...
    public function orlike($options) {
		...
		if(C('ht_orlike_shop_type')){$map['shop_type'] = C('ht_orlike_shop_type');}

		if(C('ht_orlike_time') == '1'){
			$map['coupon_start_time'] = array('egt',time());
		}elseif(C('ht_orlike_time') =='2'){
			$map['coupon_start_time'] = array('elt',time());
		}
		if(C('ht_orlike_end_time') == '1'){$map['coupon_end_time'] = array('egt',time());}



		if(C('ht_orlike_ems') == '1'){$map['ems'] = '1';}

		if(C('ht_orlike_mix_price')>0){$map['coupon_price'] = array('egt',C('ht_orlike_mix_price'));}
		if(C('ht_orlike_max_price')>0){$map['coupon_price'] = array('elt',C('ht_orlike_max_price'));}
		if(C('ht_orlike_mix_price')>0 && C('ht_orlike_max_price')>0){$map['coupon_price'] =  array(array('egt',C('ht_orlike_mix_price')),array('elt',C('ht_orlike_max_price')),'and');}
		if(C('ht_orlike_mix_volume')>0){$map['volume'] = array('egt',C('ht_orlike_mix_volume'));}
		if(C('ht_orlike_max_volume')>0){$map['volume'] = array('elt',C('ht_orlike_max_volume'));}
		if(C('ht_orlike_mix_volume')>0 && C('ht_orlike_max_volume')>0){$map['volume'] = array(array('egt',C('ht_orlike_mix_volume')),array('elt',C('ht_orlike_max_volume')),'and');}

		$data = $items_mod->where($map)->limit('0,'.C('ht_orlike_page_size').'')->order(C('ht_orlike_sort'))->select();
        return $data;
    }
	...
	public function zhi($options) {
		...
		if(C('ht_zhi_shop_type')){$map['shop_type'] = C('ht_zhi_shop_type');}

		if(C('ht_zhi_time') == '1'){
			$map['coupon_start_time'] = array('egt',time());
		}elseif(C('ht_zhi_time') =='2'){
			$map['coupon_start_time'] = array('elt',time());
		}
		if(C('ht_zhi_end_time') == '1'){$map['coupon_end_time'] = array('egt',time());}

		if(C('ht_zhi_ems') == '1'){$map['ems'] = '1';}

		if(C('ht_zhi_mix_price')>0){$map['coupon_price'] = array('egt',C('ht_zhi_mix_price'));}
		if(C('ht_zhi_max_price')>0){$map['coupon_price'] = array('elt',C('ht_zhi_max_price'));}
		if(C('ht_zhi_mix_price')>0 && C('ht_zhi_max_price')>0){$map['coupon_price'] =  array(array('egt',C('ht_zhi_mix_price')),array('elt',C('ht_zhi_max_price')),'and');}

		if(C('ht_zhi_mix_volume')>0){$map['volume'] = array('egt',C('ht_zhi_mix_volume'));}
		if(C('ht_zhi_max_volume')>0){$map['volume'] = array('elt',C('ht_zhi_max_volume'));}
		if(C('ht_zhi_mix_volume')>0 && C('ht_zhi_max_volume')>0){$map['volume'] = array(array('egt',C('ht_zhi_mix_volume')),array('elt',C('ht_zhi_max_volume')),'and');}

		$data = $items_mod->where($map)->limit('0,'.$options['num'])->order(C('ht_zhi_sort'))->select();
        return $data;
    }
	...
	public function lists($options) {
        ...
        $data = $items_mod->where($map)->limit('0 ,' . $options['num'])->order(C('ht_index_sort'))->select();
		return $data;
	}
