...
class TopClient{
	public $appkey;
	public $secretKey;
	public $gatewayUrl = "http://api2.htian.com/api/";
	public $format = "json";
	/** 是否打开入参check**/
	...
	public $_CachePath = HT_DATA_PATH   ;
	public $_method = "";
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt){
		...
		$logger = new LtLogger;
		$logger->conf["log_file"] = $this->_CachePath . "htian/top_comm_err_" . $this->appkey . "_" . date("Y-m-d") . ".log";
	...
	public function execute($request, $session = null){
		...
		$this->_CachePath = $this->_CachePath . "runtime/HtianAPI/";
		if(!$this->appkey){
			$result = '{"errors":{"error":{"msg":"Missing App Key","err_code":"10002"}}}';
			$logger = new LtLogger;
			$logger->conf["log_file"] = $this->_CachePath . "htian_logs/top_biz_err_" . $this->appkey . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
				date("Y-m-d H:i:s"),
				$this->_method,
				$result
		...
		if (!$resp = $this->getCacheData($cacheid)) {
			//发起HTTP请求
			$logger = new LtLogger;
			$logger->conf["log_file"] = $this->_CachePath . "htian_logs/logs_" . $this->appkey . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
				$this->appkey,
				$this->_method,
				date("Y-m-d H:i:s")
