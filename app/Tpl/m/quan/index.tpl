...
<div class="app">
    <header class="head" id="head">
        ...
        <dl class="screen-box">
		<if condition="$is_so eq 0">
            <dt>筛选方式：</dt>
            <dd>
            <a href="{:U('quan/index',array('sort'=>'new'))}" title="最新" >最新<img  <if condition="$index_info['sort'] eq 'new'">style="display:block;" </if>src="__STATIC__/jwap/images/selected.png">
            </a>
            </dd>
            <dd>
            <a href="{:U('quan/index',array('sort'=>'hot'))}" title="最热">最热<img  <if condition="$index_info['sort'] eq 'hot'">style="display:block;" </if>src="__STATIC__/jwap/images/selected.png">
            </a>
            </dd>
             <dd>
            <a href="{:U('quan/index',array('sort'=>'price'))}" title="价格">价格<img  <if condition="$index_info['sort'] eq 'price'">style="display:block;" </if>src="__STATIC__/jwap/images/selected.png">
            </a>
            </dd>
             <dd>
            <a href="{:U('quan/index',array('sort'=>'rate'))}" title="折扣">折扣<img  <if condition="$index_info['sort'] eq 'rate'">style="display:block;" </if>src="__STATIC__/jwap/images/selected.png">
            </a>
            </dd>
           <else/>
            <dt>筛选方式：</dt>
            <dd>
            <a href="{:U('quan/so',array('sort'=>'new','k'=>$skey))}" title="最新" >最新<img  <if condition="$index_info['sort'] eq 'new'">style="display:block;" </if>src="__STATIC__/jwap/images/selected.png">
            </a>
            </dd>
            <dd>
            <a href="{:U('quan/so',array('sort'=>'hot','k'=>$skey))}" title="最热">最热<img  <if condition="$index_info['sort'] eq 'hot'">style="display:block;" </if>src="__STATIC__/jwap/images/selected.png">
            </a>
            </dd>
             <dd>
            <a href="{:U('quan/so',array('sort'=>'price','k'=>$skey))}" title="价格">价格<img  <if condition="$index_info['sort'] eq 'price'">style="display:block;" </if>src="__STATIC__/jwap/images/selected.png">
            </a>
            </dd>
             <dd>
            <a href="{:U('quan/so',array('sort'=>'rate','k'=>$skey))}" title="折扣">折扣<img  <if condition="$index_info['sort'] eq 'rate'">style="display:block;" </if>src="__STATIC__/jwap/images/selected.png">
            </a>
            </dd>		   
		   </if>
        ...
        <div id="search-box">
           ...
           <form id="search-form" action="{:C('ht_header_html')}" method="get">
				...
				<input type="hidden" name="a" value="so" />
                    ...
                    <input type="text" id="k" name="k" x-webkit-speech="" placeholder="搜索优惠券商品..." autocomplete="off" value="">
...
<volist name='items_list' id='item'> 		
		...
		<p class="pHeader"><a href="{:U('item/index',array('id'=>$item['id']))}" target="_self" rel="nofollow">{$item.title}</a><span class="glyphicon"> 优惠券：￥</span><b>{$item.quan}元</b></p>
        ...
        <p class="pHeader"><a href="{:U('item/index',array('id'=>$item['id']))}" target="_self" rel="nofollow"></a>
		
		<if condition="$item.isq eq '1' "><div class="divPrice">券后价￥<span>{$item.coupon_price}</span>在售价￥<span>{$item.price}</span></div></if>
		<div class="divGo"><a href="{:U('item/index',array('id'=>$item['id']))}" target="_self" rel="nofollow"><span>①领券</span></a>
        <span>→</span>
		<a href="{:U('item/index',array('id'=>$item['id']))}" target="_blank" rel="nofollow"><span>②下单</span></a></div>     
		</li> 				
</volist>	
</ul>
</section>
