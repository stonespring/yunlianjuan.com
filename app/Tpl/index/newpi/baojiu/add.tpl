...
<title>{:C('ht_site_name')} - {$page_seo.title} 商家报名中心</title>
...
<div class="main {:C('ht_site_width')} mb20">
...
function getTaoItem(url){
	...
	$.getJSON(HTIANER.root + '/?m=baoming&a=ajaxGetItem', {url:url}, function(result){
	if(typeof result.status=='0'){
			...
			return false;
								...
								$('#pic_url').val("http:"+result.data.pic_url);
...
$("#smt").click(function() {
	...
	$.ajax({
            url: HTIANER.root + '/index.php?m=baoming&a=ajaxadd',
            type: 'POST',
            data: {
            iid: iid,
            cate_id: cate_id,
