...
<div class="main w1200">
	<div class="place-explain">
		您的位置：<a target="_blank" href="{:C('ht_site_url')}">{:C('ht_site_name')}</a>
		&nbsp;&gt;&nbsp;			
	...
	<div class="container fl">
		<div class="product clear">
			<div class="product-pic fl">
				<a <eq name="item.quanurl" value=""> href="{:U('jump/index',array('id'=>$item['id']))}" <else/>href="{$item.quanurl}" </eq> target="_blank" rel="nofollow"><img src="{:attach(get_thumb($item['pic_url'], '_b'),'item')}" alt="{$item.title}" /></a>				
			</div>
			<div class="product-info fl">
				<h3><a <eq name="item.quanurl" value=""> href="{:U('jump/index',array('id'=>$item['id']))}" <else/>href="{$item.quanurl}" </eq> target="_blank" rel="nofollow"><div data-plugin="keyword">{$item.title}</div></a></h3>
				...
				<div class="fanli-price fl">
                <div class="fanli-current" ><div class="ft" >{:C('ht_first_word')}</div></div>
				<div class="fanli-current1" ><div class="ft1" >								
								{:C('ht_second_word')}	

				</div></div>
				<span class="fr"><a href="{:U('help/read',array('id'=>7))}" target="_blank" ><font size="2" color="blue">[查看返利规则]</font></span>
                </div>
					</if>				
				<div class="pg"></div>
				...
				<div class="share"><a class="buy_ed report" znid="{$item.id}" title="{$item.title}" href="javascript:;"><i class="report-icon"></i>举报</a></div>
					<span class="btn-tip fl">						
						...
						<elseif condition="$item.class eq 'buy'"/>
							<a class="fl go_btn" <eq name="item.quanurl" value=""> href="{:U('jump/index',array('id'=>$item['id']))}" <else/>href="{$item.quanurl}" </eq> target="_blank">
             ...
             <if condition="$item.isq eq 1">  <div class="btn_buy" _hover-ignore="1">
						购买流程：<a href="{$item.quanurl}" target="_blank" rel="nofollow"><span _hover-ignore="1">①点我领券</span></a><span>&gt;</span><a <eq name="item.quanurl" value=""> href="{:U('jump/index',array('id'=>$item['id']))}" <else/>href="{$item.quanurl}" </eq> rel="nofollow" target="_blank"><span>②点击下单</span></a>
					</div>			</if>
				</div>
				...
				<p class="price bady-time">
				  <if condition="$item.class eq 'start' ">
					...
					<span class="common nomind shoot_time"><a href="http://qzs.qq.com/snsapp/app/bee/widget/open.htm#content=亲，您在 {:C('ht_site_name')} 关注的【{$item.title}】马上就要开始抢购了哦。&time={$item.coupon_start_time|date='20y-m-d H:i',###}&advance=5&url={$item.txurl}" target="_blank">设置提醒</a></span>
                <else />				  
                <if condition="$item.class eq 'gone' ">
					<span class="fl">结束时间：</span>
					<span class="buy_time fl">{$item.coupon_start_time|date="m月d日 H时i分",###}</span>
					<span class="common nomind shoot_time"><a href="http://qzs.qq.com/snsapp/app/bee/widget/open.htm#content=亲，您在 {:C('ht_site_name')} 关注的【{$item.title}】下期活动马上就要开始了哦。&time={$item.coupon_end_time|date='20y-m-d H:i',###}&advance=5&url={$item.txurl}" target="_blank">下期提醒</a></span>	                	
				<else />
                <if condition="$item.class eq 'buy' ">
					...
					<span class="common nomind shoot_time"><a href="http://qzs.qq.com/snsapp/app/bee/widget/open.htm#content=亲，您在 {:C('ht_site_url')} 关注的【{$item.title}】优惠活动马上就要结束了哦。&time={$item.coupon_end_time|date='20y-m-d H:i',###}&advance=5&url={$item.txurl}" target="_blank">结束提醒</a></span>	
                </if>	
                </if>		
                </if>
				</p>
		...
		<div class="ds-thread" data-thread-key="{$item.id}" data-title="{$item.title}" data-url="{:C('ht_site_url')}index.php?m=item&a=index&id={$item.id}"></div>  
...
<load href="__STATIC__/htian/js/report.js" />
