...
<div class="header">
    <div class="area">
		...
		<form name="search" action="{:C('ht_site_url')}" method="get" id="search">
		   ...
		   <input x-webkit-speech name="k" id="k" autocomplete="off" onblur="this.value==''?this.value=this.title:null" onfocus="this.value==this.title?this.value='':null" title="请输入想找的宝贝" value="请输入想找的宝贝" class="txt" style="color:#303030" />
    ...
    <div class="mainNav">
        <div class="nav">
			...
			<li <if condition="$nav_curr eq $val['alias']">class='current'</if>><a href="{$val.link}" <if condition="$val.target eq 1">target="_blank"</if>>{$val.name}</a></li>
             ...
             <li>
                    <a href="{:U('apps/index')}" target="_blank" id="show-qcodes">
			...
			<a href="#" class="sign_btn signin normal-a"><em class="icon-normal icon-sign"></em><em class="text">签到领积分</em></a>
                <div class="normal-side-box">
                    <div class="box-tips">
		                ...
		                <p>每日登录<b>+{:C('ht_score_rule.login')}</b>积分</p>
