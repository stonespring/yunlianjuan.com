...
<div class="pad_lr_10">
    ...
    <table width="100%" class="table_form contentWrap">
		<tr>
            <th width="100">大淘客Appkey :</th>
            <td>
			                <label><input type="text" name="setting[dataoke]" class="input-text" size="20" value="{:C('ht_dataoke')}"><a href="http://houtianbest.cn/forum.php?mod=viewthread&tid=66&extra=page%3D1" target="_blank">查看教程</a></label>
            </td>
        </tr>
        <tr>
            <th></th>
...
function secret(secret){
	secret=$('#secret').val()
	$.getJSON("{:U('houtianyouhuiquan/secret')}",{secret:secret}, function(result){
		$("#TS").empty();
		if (result.ts == 'OK'){
			$("#TS").append('保存成功！');
		}else{
...
function SC(id){
	$.getJSON("{:U('houtianyouhuiquan/delete')}",{id:id}, function(result){
		$("#TS").empty();
		if (result.ts == 'OK'){
			$("#TS").append('删除成功！');
		}else{
...
function CJ(cate,cate1){
	$.getJSON("{:U('houtianyouhuiquan/add')}",{cate:cate,cate1:cate1}, function(result){
		$("#TS").empty();
		if (result.ts == 'OK'){
			$("#TS").append('添加成功！');
		}else{
