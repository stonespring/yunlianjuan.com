		   ...
		   <option value="0">无</option>
			...
			<tr>
				...
				<td>
					...
					<span class="gray m110" style="margin-left:2px;">登录阿里妈妈后台自助推广中获取。如果填写推广链接，上面一项是否鹊桥请选择“是”</span>
                </td>
			</tr>
			<tr>
				<th>美丽说wap推广链接 :</th>
			...
			<tr>
				...
				<th>淘口令 :</th>
				<td>
                	<input type="text" name="tao_kou_ling" id="J_tao_kou_ling" class="input-text" size="20" value="{$info.tao_kou_ling}">
...
$(function() {
  ...
  //上传图片
	...
	$('#J_gettags').live('click', function() {
		...
		if(title == ''){
			$.htian.tip({content:lang.article_title_isempty, icon:'alert'});
			return false;
		}
		...
		$('#J_get_info').live('click', function() {
			...
			//ajax获取数据
			  $.getJSON('{:U("items/ajaxGetItem")}',{link:link},function(result){
                ...
                {
                    $.htian.tip({icon:'alert',content:data.msg});
                }
            });
