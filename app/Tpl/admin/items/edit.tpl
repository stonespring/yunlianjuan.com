		...
		<table width="100%" cellpadding="2" cellspacing="1" class="table_form">
		   ...
		   <option value="0" <if condition="$info.sex eq '0'"> selected</if>>无</option>
			...
			<tr>
				<th>淘口令 :</th>
				<td>
                	<input type="text" name="tao_kou_ling" id="tao_kou_ling" class="input-text" size="20" value="{$info.tao_kou_ling}">
                </td>
			</tr>	
                        			<tr>
...
$(function() {	
  ...
  //上传图片
	...
	$('#J_gettags').live('click', function() {
		...
		if(title == ''){
			$.htian.tip({content:lang.title_empty, icon:'alert'});
			return false;
		}
	...
	$('#J_getclick_url').live('click', function() {
		...
		if(iid == ''){
			$.htian.tip({content:lang.iid_empty, icon:'alert'});
			return false;
		}
