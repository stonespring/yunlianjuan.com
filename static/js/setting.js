...
;(function(){
    $.htian.setting = {
        init: function(){
            $.htian.setting.basic();
            $.htian.setting.password();
            $.htian.setting.union();
            $.htian.setting.message_list();
            $.htian.setting.message();
            $.htian.setting.target();
        },
        //上传头像
        basic: function(){
            ...
            $('#J_upload_avatar').uploader({
                action_url: HTIANER.root + '/?m=user&a=upload_avatar',
                input_name: 'avatar',
		...
		union: function(){
            $('#links_btn').live('click', function(){
				...
				sibling_input.select();
				 $.htian.setting.copy(sibling_input.val(),'ok');
            });
			...
			$('input.links_in').blur(function(){
				if($(this).data('focus')){
					 $.htian.setting.copy($(this).val(),'not');
					$(this).data('focus',0);
				}
			});
        },
        ...
        message: function(){
            $('#J_msg_send').live('click', function(){
                ...
                $.ajax({
                    url: HTIANER.root + '/?m=message&a=publish',
                    type: 'POST',
                    data: {
                        to_id: to_id,
                        content: content
        ...
        message_list: function(){
            ...
            $('#J_msg_list').find('.J_ml_del').live('click', function(){
                ...
                $.getJSON(HTIANER.root + '/?m=message&a=del', {mid:mid}, function(result){
                    if (result.status == '1') {
        ...
        target: function(){
            $('#J_search_target').live('click', function(){
                ...
                $.ajax({
                    url: HTIANER.root + '/?m=message&a=search_target',
                    type: 'POST',
                    data: {
