;(function($){
    $.htian.item = {
        ...
        fetch: function(result){
            ...
            $.htian.item.fetch_form($('#J_fetch_item'), result.item);
        },
        fetch_form: function(form, item){
            form.find('.J_pub_btn').die('click').live('click', function(){
                ...
                $.ajax({
                    url: HTIANER.root + '/?m=item&a=publish_item',
                    type: 'POST',
                    data: {
                        item: item,
                        album_id: album_id,
		...
		check_form: function(form){
			form.find('.J_check_btn').die('click').live('click', function(){
                ...
                if(!$.htian.util.isURl(item_url)) return $.htian.tip({content:lang.please_input+lang.correct_itemurl, icon:'alert'}), !1;
                $.dialog.get('check_item').title(false).content('<div class="d_loading">'+lang.wait+'</div>');
                $.getJSON(HTIANER.root + '/?m=ajax&a=fetch_item', {url:item_url}, function(result){
                    if(result.status == 1){
		...
		report: function(form){
			form.find('.J_check_btn').die('click').live('click', function(){
                var item_url = $.trim($('.item_url').val());
                $.dialog.get('check_item').title(false).content('<div class="d_loading">'+lang.wait+'</div>');
                $.getJSON(HTIANER.root + '/?m=ajax&a=report', {url:item_url}, function(result){
                    if(result.status == 1){
						$.dialog.get('check_item').close();
						$.dialog({id:'check_item_result', title:lang.chaxun, content:result.data.html, padding:'', fixed:true, lock:true});
                    }else{
