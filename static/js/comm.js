...
;(function($){
	//提示
	if('close'==$.htian.util.getCookie('leftimg')){
	...
	$('.iknow').live('click', function(){
		$('.tips_goods').css('display','none');
		$.htian.util.setCookie('leftimg', 'close');
		$('.tips_goods').remove();
	});
    ...
    function() {
        ...
        if (flag == true && $(document).scrollTop() > temp) {
            ...
            if ("fold" == $.htian.util.getCookie("fold")) {
    ...
    function() {
        ...
        $('.white_dh #fold').attr('class', 'filter-unfold');
        $.htian.util.setCookie("fold", 'fold');
    });
	...
	var F_s = function() {
			if (document.getElementById("k")) {
				document.getElementById("k").onclick = function() {
					...
					var ekv = HTIANER.site +'/?m=search&a=etao&key=' + val;
					window.location.href = HTIANER.site +'/?m=search&a=index&key=' + val;
					return false;
				}
			}
